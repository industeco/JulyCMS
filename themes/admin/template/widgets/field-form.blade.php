<el-form ref="field_{{ $mode }}_form"
  ...
  label-width="108px">
  @if ($mode == 'create')
  <el-form-item label="字段类型" prop="field_type" size="small" :class="{'has-helptext': !!{{ $model }}.parameters__field_type}">
  ...
  <el-form-item label="帮助文本" size="small">
    <el-input
      v-model="{{ $model }}.parameters__helptext"
      type="textarea"
      rows="3"
      maxlength="255"
      show-word-limit></el-input>
  ...
  <el-form-item label="可检索" size="small">
    <el-switch v-model="{{ $model }}.parameters__is_searchable"></el-switch>
  </el-form-item>
  <el-form-item v-if="{{ $model }}.parameters__is_searchable"
    label="索引权重" size="small">
    <el-input-number
      v-model="{{ $model }}.parameters__weight"
      size="small"
      controls-position="right"
      :min="1" :step="1" :precision="0"></el-input-number>
  </el-form-item>
  @if ($mode == 'create')
  <el-form-item label="最大字数" size="small" class="has-helptext"
    v-if="{{ $model }}.parameters__field_type=='text'">
    <el-input-number
      v-model="{{ $model }}.parameters__max"
      size="small"
      controls-position="right"
      :min="0"
  ...
  <el-form-item v-if="{{ $model }}.parameters__field_type=='file'" prop="parameters__field_type" label="文件类型" size="small" class="has-helptext">
    <el-select
      v-model="{{ $model }}.parameters__file_type"
      placeholder="--选择文件类型--">
      <el-option
  ...
  @endif
  <el-form-item v-if="{{ $model }}.parameters__field_type=='text'" label="默认值 [{{ $content_langcode }}]" size="small" class="has-helptext">
    <el-input
      v-model="{{ $model }}.parameters__default"
      type="textarea"
      rows="3"
      maxlength="255"
      show-word-limit></el-input>
  ...
  <el-form-item v-if="{{ $model }}.parameters__field_type=='text'" label="预选值 [{{ $content_langcode }}]" size="small" class="has-helptext">
    <div class="jc-table-wrapper">
      <table class="jc-table jc-dense is-editable with-operators with-line-number">
        ...
        <tbody
          is="draggable"
          v-model="{{ $model }}.parameters__datalist"
          :animation="150"
          ghost-class="jc-drag-ghost"
          handle=".jc-drag-handle"
          tag="tbody">
          <tr v-for="(line, index) in {{ $model }}.parameters__datalist" :key="index">
            ...
            <td>
              <div class="jc-operators">
                ...
                <button
                  ...
                  :disabled="{{ $model }}.parameters__datalist.length < 2"
