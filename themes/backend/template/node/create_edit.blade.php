...
@section('h1')
  {{ $context['edit_mode'] }}内容 <span id="content_locale">[ {{ $node_type['label'] }}({{ $node_type['id'] }}), {{ lang($langcode)->getName() }}({{ $langcode }}) ]</span>
@endsection
...
@section('main_content')
  <el-form id="main_form" ref="main_form"
    ...
    <div id="main_form_left">
      @foreach ($fields['selected'] as $field)
      {!! $field['element'] !!}
      @endforeach
    ...
    <div id="main_form_right">
      ...
      <el-collapse :value="expanded" @change="handleCollapseChange">
        ...
        <el-collapse-item title="META 信息" name="META 信息">
          {!! $fields['global']['meta_title']['element'] !!}
          {!! $fields['global']['meta_keywords']['element'] !!}
          {!! $fields['global']['meta_description']['element'] !!}
          {!! $fields['global']['meta_canonical']['element'] !!}
        </el-collapse-item>
      </el-collapse>
    </div>
  </el-form>
...
@foreach (array_merge($fields['selected'], $fields['global']) as $field)
    ...
    data() {
      ...
      return {
        node: {
          ...
          @foreach (array_merge($fields['selected'], $fields['global']) as $field)
          @if ($field['field_type_id']=='html')
          {{ $field['id'] }}: getHtmlFieldValue("{{ $field['id'] }}"),
          @else
          {{ $field['id'] }}: `{!! $field["value"] !!}`,
          @endif
          @endforeach
          tags: @json($node['tags'], JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE),
        },
