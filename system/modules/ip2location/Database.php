...
class Database
{
     ...
     * 免费数据库
     */
    private static $liteDatabases = [
        'IP2LOCATION-LITE-DB3.IPV6',
	...
	private static function findFile($file = null)
	{
		...
		foreach (self::$liteDatabases as $database) {
			$rfile = realpath("{$current}/databases/{$database}.BIN");
			if ($rfile !== false) {
				return $rfile;
			}
		}
