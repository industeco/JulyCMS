...
<el-form id="main_form" ref="main_form"
  ...
  <div id="main_form_left">
    ...
    <el-form-item prop="{{ $key }}" size="small"
      ...
      <el-tooltip slot="label" popper-class="jc-twig-output" effect="dark" content="@{{ config('{{ $key }}') }}" placement="right">
    ...
    methods: {
      ...
      submit() {
        ...
        form.validate().then(() => {
          ...
          axios.post("{{ short_url('settings.update', $name) }}", settings).then(response => {
            loading.close();
            this.initial_data = clone(this.settings);
