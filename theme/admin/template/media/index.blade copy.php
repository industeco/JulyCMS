...
@section('main_content')
  <div id="media-manager">
    ...
    <el-dialog
      ...
      <div class="jc-upload">
        <ul class="jc-upload__file-list md-scrollbar md-theme-default"
          :class="{
            'is-uploading': upload.status === 'uploading',
            'is-complete': upload.status === 'uploaded',
          }">
          <li v-for="file in upload.fileList" :key="file.name" class="jc-upload__file">
            <img :src="file.url" alt="" :title="file.name">
            <button type="button" title="移除" class="md-button md-fab md-mini md-accent md-theme-default" @click="handleUploadFileRemove(file)">
              <div class="md-ripple">
                <div class="md-button-content"><i class="md-icon md-icon-font md-theme-default">close</i></div>
              </div>
            </button>
        ...
        <el-upload
          ...
          :auto-upload="false"
          :on-change="handleUploadFileListChange"
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <button type="button" class="md-button md-raised md-dense md-primary md-theme-default"
          :disabled="upload.status === 'uploading' || (upload.status === 'standingby' && !upload.fileList.length)"
          @click="handleSubmitUpload">
          <div class="md-ripple">
            <div class="md-button-content">@{{ upload.status==='uploaded'?'关 闭':'上 传' }}</div>
          </div>
        </button>
      </span>
    </el-dialog>
  ...
  let app = new Vue({
    ...
    methods: {
      ...
      closeUpload() {
        ...
        this.$set(this.$data.upload, 'fileList', []);
      ...
      getUploadFile(file) {
        let target;
        this.upload.fileList.every(item => {
      ...
      handleUploadFileListChange(file, fileList) {
        if (this.upload.status === 'standingby') {
          if (this.getUploadFile(file.raw) || !this.checkUploadFile(file.raw)) {
            this.$refs.media_upload.handleRemove(null, file);
          } else {
      ...
      checkUploadFile(file) {
        ...
        if (this.allowedMimeTypes.indexOf(file.type) < 0) {
          console.log('文件类型检查未通过')
          allowed = false;
        }

        // 检查文件大小
        else if (file.size/1024/1024 > 2) {
          console.log('文件大小检查未通过')
          allowed = false;
        }

        // 检查文件是否已存在
        else if (this.getCurrentFile(file)) {
          console.log('文件已存在')
          allowed = false;
        }
      ...
      handleSubmitUpload() {
        if (this.upload.status === 'uploaded' || !this.upload.fileList.length) {
          this.closeUpload();
        } else {
          this.upload.uploaded = 0;
          this.upload.status = 'uploading';
          this.upload.postData.cwd = this.currentPath
          this.$refs.media_upload.submit();
        }
      },

      handleUploadProgress(event, file, fileList) {
        const target = this.getUploadFile(file);
        if (target) {
      ...
      handleUploadSuccess(response) {
        this.upload.response = _.extend(this.upload.response, response);
        this.upload.uploaded++;
        if (this.upload.uploaded >= this.upload.fileList.length) {
